...
const CustomerDashboard: React.FC = () => {
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-16">
      {/* Tab Navigation */}
      <div className="flex flex-wrap items-center gap-2 mb-2 bg-slate-200/50 p-1.5 rounded-2xl w-max">
        <button onClick={() => setActiveTab('overview')} className={cn("px-4 sm:px-6 py-2.5 rounded-xl text-sm font-bold flex items-center gap-2 transition-all", activeTab === 'overview' ? "bg-white text-indigo-600 shadow-sm" : "text-slate-500 hover:text-slate-700 hover:bg-slate-200")}>
          <LayoutDashboard className="w-4 h-4" /> Overview
        </button>
        <button onClick={() => setActiveTab('tasks')} className={cn("px-4 sm:px-6 py-2.5 rounded-xl text-sm font-bold flex items-center gap-2 transition-all", activeTab === 'tasks' ? "bg-white text-indigo-600 shadow-sm" : "text-slate-500 hover:text-slate-700 hover:bg-slate-200")}>
          <CheckSquare className="w-4 h-4" /> Tasks
        </button>
        <button onClick={() => setActiveTab('support')} className={cn("px-4 sm:px-6 py-2.5 rounded-xl text-sm font-bold flex items-center gap-2 transition-all", activeTab === 'support' ? "bg-white text-indigo-600 shadow-sm" : "text-slate-500 hover:text-slate-700 hover:bg-slate-200")}>
      ...
      {activeTab === 'tasks' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-fade-in">
          <div className="lg:col-span-2 space-y-6">
            <div className="glass p-6 rounded-3xl border border-white/40 shadow-sm min-h-[400px]">
              <h3 className="text-lg font-bold text-slate-900 mb-6">My Tasks & Requests</h3>
              {isTasksLoading ? (
                 ...
                 <div className="space-y-4">
                   {myTasks.map(task => (
                      <div key={task.id} className="p-4 rounded-2xl bg-white/60 border border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:shadow-md transition-shadow">
                         <div>
                            <h4 className="font-bold text-slate-800">{task.title}</h4>
                            <div className="flex flex-wrap gap-4 mt-2 text-xs font-medium text-slate-500">
                               <span className="flex items-center gap-1.5"><Calendar className="w-3.5 h-3.5 text-indigo-500" /> Due: {new Date(task.dueDate).toLocaleDateString()}</span>
                               <span className="flex items-center gap-1.5"><Tag className="w-3.5 h-3.5 text-amber-500" /> Priority: {task.priority}</span>
                               <span className="flex items-center gap-1.5"><AlertCircle className="w-3.5 h-3.5 text-emerald-500" /> Status: {task.status}</span>
                            </div>
                         </div>
                         <div className="sm:text-right shrink-0 bg-slate-50 p-2 rounded-xl">
                            <span className="text-[10px] uppercase font-bold text-slate-400 block mb-1">Assigned To</span>
                            <span className={cn(
                              "text-xs font-bold px-2 py-1 rounded-md inline-block",
                              task.assignedTo === 'Pending Allocation' ? "text-amber-600 bg-amber-100" : "text-indigo-600 bg-indigo-100"
                            )}>
                              {task.assignedTo}
                            </span>
                         </div>
          ...
          <div className="space-y-6">
            <div className="glass p-6 rounded-3xl border border-white/40 shadow-sm">
              <h3 className="text-sm font-bold text-slate-900 uppercase tracking-widest opacity-40 mb-6">Create New Request</h3>
              <form onSubmit={handleCreateTask} className="space-y-4">
                 <div>
                   ...
                   <input 
                     ...
                     className="w-full bg-white/60 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition-all placeholder-slate-400" 
                     placeholder="What needs to be done?" 
                   />
                 </div>
                 <div>
                   <label className="block text-xs font-bold text-slate-700 mb-1.5">Expected Due Date</label>
                   <input 
                     type="date" 
                     required 
                     value={newTaskDueDate} 
                     onChange={e => setNewTaskDueDate(e.target.value)} 
                     className="w-full bg-white/60 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition-all" 
                   />
                 </div>
                 <div>
                 ...
                 <button 
                   type="submit" 
                   className="w-full mt-4 bg-indigo-600 text-white rounded-xl py-3.5 text-sm font-bold flex items-center justify-center gap-2 hover:bg-indigo-700 shadow-xl shadow-indigo-100 transition-all"
                 >
