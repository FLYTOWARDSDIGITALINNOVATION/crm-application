...
import { X, User, Mail, Phone, Building2, Globe, Tag, Save } from 'lucide-react';
...
const EditLeadModal: React.FC<EditLeadModalProps> = ({ isOpen, onClose, lead }) => {
  ...
  // Parse source if it contains Software Type
  const initialSource = lead.source?.startsWith('Software - ') ? 'Software' : (lead.source || 'Marketing');
  const initialSoftwareType = lead.source?.startsWith('Software - ') ? lead.source.replace('Software - ', '') : 'Billing';
  ...
  const { register, handleSubmit, formState: { errors }, reset, watch } = useForm<FormData>({
    ...
    defaultValues: {
      ...
      softwareType: initialSoftwareType,
      notes: lead.notes || '',
    }
  });
  ...
  const onSubmit = async (data: FormData) => {
    const finalData: Partial<Lead> = { ...data } as any;
    if (data.source === 'Software' && data.softwareType) {
      finalData.source = `Software - ${data.softwareType}`;
    }
    delete (finalData as any).softwareType;

    try {
      await dispatch(updateLead({ id: lead._id, data: finalData })).unwrap();
      onClose();
    } catch (err) {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-2xl bg-white rounded-3xl shadow-2xl overflow-hidden animate-slide-up">
        <div className="px-8 py-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <div>
            <h2 className="text-xl font-bold text-slate-900 font-inter">Edit Lead</h2>
            <p className="text-slate-500 text-sm mt-0.5">Update the details for this lead.</p>
          </div>
          <button 
            onClick={onClose}
        ...
        <form id="edit-lead-form" onSubmit={handleSubmit(onSubmit)} className="p-8 space-y-6 max-h-[70vh] overflow-y-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <div className="relative group">
                ...
                <select
                  {...register('status')}
                  className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 outline-none appearance-none transition-all cursor-pointer"
                >
